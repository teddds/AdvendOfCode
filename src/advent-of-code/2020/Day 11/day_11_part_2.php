...
class Map
{
	...
	public function __construct(string $input)
	{
		...
		foreach ($rows as $index => $row) {
			...
			$this->width = count($row_array);
			foreach ($row_array as $col => $char) {
		...
		foreach ($this->map as $row_index => $row) {
			foreach ($row as $col_index => $item) {
				...
				foreach ($adjacents as $adjacent) {
					if ($adjacent->state === Position::STATE_OCCUIPIED) {
						++$adjacentCnt;
					}
				}

				if ($item->state === Position::STATE_EMPTY && $adjacentCnt === 0) {
					$changed = true;
					$item->preState = Position::STATE_OCCUIPIED;
				} elseif ($item->state === Position::STATE_OCCUIPIED && $adjacentCnt >= 5) {
					$changed = true;
	 ...
	 * @return Position[]
	 */
	private function getAdjacents(int $row, int $col): array
	{
		$adjacents = [];
		$adjacents[] = $this->getAdjecentByOperator($col, $row, -1, 0);
		$adjacents[] = $this->getAdjecentByOperator($col, $row, -1, -1);
		$adjacents[] = $this->getAdjecentByOperator($col, $row, 0, -1);
		$adjacents[] = $this->getAdjecentByOperator($col, $row, 1, -1);
		...
		return array_merge([], ...$adjacents);
	}

	private function getAdjecentByOperator(int $start_x, int $start_y, int $change_x, int $change_y)
	{
		$adjacents = [];
		$x = $start_x;
		$y = $start_y;
		while (true) {
			...
			if ($this->map[$y][$x] instanceof Seat) {
